...
//==================================================================================================================
        ...
        /// Calculate Tangents
        /// </summary>
        /// <param name="chunkSettings"></param>
        /// <param name="meshData"></param>
        /// <param name="jobHandle"></param>
        /// <returns></returns>
...
//--------------------------------------------------------------------------------------------------------------------------
        ...
        public struct JNormals : IJobFor
        {
            ...
            private float3 GetNormal(int x, int y, int width, bool isRightTriangleBottom) //quad split from bottom-right to top-left
            {
                ...
                if (IsCellOnCorner(x, y, lastIndexWidth))
                {
                    ...
                    normal = (x,y) switch
                    {
                        (0, 0) => GetNormalFromTopRight(x, y, width),
                        (_, 0) when x == lastIndexWidth => GetNormalFromTopLeft(x, y, width),
                        (0, _) when y == lastIndexWidth => GetNormalFromBottomRight(x, y, width),
                        _ => GetNormalFromBottomLeft(x, y, width)
                    };
                    */
                ...
                else
                {
                    numTriangleVertex = 6;
                    normal = GetNormalFromTopRight(x, y, width) 
                             + GetNormalFromBottomRight(x, y, width) 
                             + GetNormalFromTopLeft(x, y, width) 
                             + GetNormalFromBottomLeft(x, y, width);
                }
            ...
            private bool IsCellOnCorner(int x, int y, int lastIndexWidth)
            {
                return (x == 0 && y == 0) 
                       ...
                       || (x == lastIndexWidth && y == lastIndexWidth);
            }
        }
        ...
        public struct JNoiseMap : IJobFor
        {
            ...
            public void Execute(int index)
            {
                float2 halfMapSize = new (MapSizeX * 0.5f);
                float2 xy = GetXY2(index, MapSizeX);
                ...
                {
                    float2 sampleXY = xy - halfMapSize + Octaves[i];
                    sampleXY = sampleXY / Settings.Scale * frequency;
                ...
                float clampedNoise = abs(lerp(0, 1f, noiseHeight));
                NoiseMap[index] = clampedNoise * Settings.HeightMultiplier;
            }
        }
    }
}
